...
exports.getJobOrderProgress = exports.updateJobOrderStatus = exports.getJobOrderById = exports.getJobOrders = exports.createJobOrder = void 0;
...
const createJobOrder = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const [result] = yield database_1.default.query('INSERT INTO job_orders (order_number, product_name, quantity, status, due_date, special_instructions) VALUES (?, ?, ?, ?, ?, ?)', [order_number, product_name, quantity, 'Pending', due_date, special_instructions]);
...
const getJobOrderProgress = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    try {
        const [jobOrders] = yield database_1.default.query('SELECT * FROM job_orders WHERE id = ?', [id]);
        const [pallets] = yield database_1.default.query('SELECT COUNT(*) as completed FROM pallets WHERE job_order_id = ? AND status = "In Warehouse"', [id]);
        if (jobOrders.length === 0) {
            return res.status(404).json({ message: 'Job order not found' });
        }
        const jobOrder = jobOrders[0];
        const completedPallets = pallets[0].completed;
        res.json({
            jobOrder,
            completedPallets,
...
exports.getJobOrderProgress = getJobOrderProgress;
